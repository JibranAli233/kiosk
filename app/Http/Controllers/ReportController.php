...
class ReportController extends Controller
{
    ...
    public function index(Request $request)
    {
        $companies          = DB::table('companies')
                                ...
                                ->select('companies.id',
                                        DB::raw('CONCAT(companies.name) as name'))
                                ->pluck('name','id')
                                ->all();
        ...
        $customers          = DB::table('users')
                                ->orderBy('users.id')
                                ->select('users.id',
                                          DB::raw('CONCAT(users.name) as name'))
                                ->pluck('name','id')
                                ->all();
    ...
    public function store(Request $request)
    {
        ...
        // dd($request->request);  
            ...
            $from = date('Y-m-d', strtotime($request['company_start_date']));
            $to = date('Y-m-d', strtotime($request['company_end_date']));
            ...
            $rec            = DB::table('company_has_transactions')
                                ...
                                ->select(
                                        ...
                                        'purchases.order_no',
                                        'company_has_transactions.created_at'
                                        )
                                ->where('company_has_transactions.company_id', $company_id)
                                // ->whereDate('company_has_transactions.created_at', $date)
                                // ->whereBetween('company_has_transactions.created_at', [$from, $to])
                                ->whereDate('company_has_transactions.created_at', '>=', $from)
                                ->whereDate('company_has_transactions.created_at', '<=', $to)
            ...
            $rec            = DB::table('customer_has_transactions')
                                ...
                                ->leftjoin('users', 'users.id', '=', 'customer_has_transactions.customer_id')
