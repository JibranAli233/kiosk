        ...
        if($data){
            if($item){
                foreach($item as $item_key => $item_value){
                    ...
                    $tot_price          += (($purchase_price[$item_key]) * ($qty[$item_key]));
                    $var->save();
                }
            }

            $pay_amount  =  isset($inputs['pay_amount']) ? $inputs['pay_amount'] : 0;

            // $inputs['net_amount']       = $tot_price;
            $data->update([
                            'total_amount'  => $tot_price,
                            'net_amount'    => ($tot_price - $pay_amount),
                        ]); 
            ...
            $val->credit            = isset($inputs['pay_amount']) ? $inputs['pay_amount'] : 0;
            $val->debit             = $tot_price;
            $val->save();
