...
@endphp
{!! Form::open(array('method'=>$method,'id'=>'jrnl_form','enctype'=>'multipart/form-data')) !!}
    ...
    {{  Form::hidden('custom_id', (isset($data->custom_id) ? $data->custom_id : 0) , array("class" => "jv_custom_id")) }}
                ...
                <div class="card-header" style="display:none">
                    <div class="row">
                        <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 col_head">
                            {!! Html::decode(Form::label('selected_crdt_acnt_bal','Selected debit account balance')) !!}</br>
                        ...
                        <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 col_head">
                            {!! Html::decode(Form::label('selected_account_balance','Selected credit account balance')) !!}</br>
                                    ...
                                    <tbody>
                                        ...
                                        @if(isset($transactions))
                                                @foreach($transactions as $key => $transaction)
                                                    <tr>
                                                        <td>
                                                            {!! Form::select("dbt_acnt_ids[]", ["Please select"]+hp_accounts() ,$transaction->account_id, array("class" => "form-control select2 cls_dbt_acnt_id")) !!}
                                                        </td>
                                                        <td>
                                                            {{ Form::text("dbt_details[]", $transaction->detail, array("placeholder" => "Enter details","class" => "form-control")) }}
                                                        </td>
                                                        <td>
                                                            {{ Form::number("dbt_amounts[]", $transaction->ledger->amount, array("placeholder" => "amounts","class" => "form-control cls_dbt_amount","min"=>0, "step"=>"any")) }}
                                                        </td>
                                                        <td>
                            ...
                            <div class="table-responsive">
                                <table class="table table_6" id="tbl_jrnl">
                                    ...
                                    <tbody>
                                        ...
                                        @if(isset($crtransactions))
                                            @foreach($crtransactions as $key => $transaction)
                                                <tr>
                                                    <td>
                                                        {!! Form::select("account_ids[]", ["Please select"]+hp_accounts() ,$transaction->account_id, array("class" => "form-control select2 cls_jrnl_account_ids")) !!}
                                                    </td>
                                                    <td>
                                                        {{ Form::text("details[]", $transaction->detail, array("placeholder" => "Enter details","class" => "form-control")) }}
                                                    </td>
                                                    <td>
                                                        {{ Form::number("amounts[]", $transaction->ledger->amount, array("placeholder" => "amounts","class" => "form-control cls_jrnl_amnt","min"=>0, "step"=>"any")) }}
...
<script>
    $(document).ready(function () {
        ...
        if((bank_account_id != 0) && (trnx_type_id!=0)){
            getAccountCurrentBalance(bank_account_id,'selected_account_balance');
        }

        if((account_id != 0) && (trnx_type_id!=0)){
            getAccountCurrentBalance(account_id,'selected_crdt_acnt_bal');
        }
