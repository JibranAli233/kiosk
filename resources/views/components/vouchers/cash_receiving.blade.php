    ...
    <div class="col-md-12">
        <div class="card">
            ...
            <!--begin::Form-->
                ...
                @endphp
                {!! Form::open(array('method'=>$method,'id'=>'csh_rcvng_form','enctype'=>'multipart/form-data')) !!}
                                    ...
                                    <table class="table table_2" id="tbl_csh_rcvng">
                                        <thead>
                                            <tr>
                                                ...
                                                <th style="width: 10%;">
                                                    @if(!(isset($data)))
                                                        <a class="text-light btn btn-primary btn-xs add_csh_rcvng btn_add" id="">+</a>
                                                    @endif
                                                </th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            @if(isset($data))
                                                <tr>
                                                    <td>
                                                        {!! Form::select("account_ids[]", ["Please select"]+hp_accounts() ,$data->account_id, array("class" => "form-control select2 cls_csh_rcvng_account_ids")) !!}
                                                    </td>
                                                    <td>
                                                        {{ Form::text("details[]", $data->detail, array("placeholder" => "Enter details","class" => "form-control")) }}
                                                    </td>
                                                    <td>
                                                        {{ Form::number("amounts[]", $data->ledger->amount, array("placeholder" => "amounts","class" => "form-control cls_csh_rcvng_amnt","min"=>0, "step"=>"any")) }}
                                                    </td>
                                                    <td>
                                                        @if(!(isset($data)))
...
<script>
        $(document).ready(function () {  
            ...
            var transaction_id  = {{ isset($data->id) ? $data->id : 0 }};
            ...
            var account_bal     = {{ isset($data->ledger->amount) ? $data->ledger->amount : 0}}

            var trnx_type_id    = {{ ((isset($data->transaction_type_id)) && ($data->transaction_type_id == 2)) ? $data->transaction_type_id : 0 }};
            var account_id      = {{ isset($data->account_id) ? $data->account_id : 0}}
            ...
            $(document).on('change', '.cls_csh_rcvng_amnt', function() {
                ...
                var balance = (cih_balance -account_bal) + parseFloat(amount); // Convert amount to a number and add it to cih_balance
