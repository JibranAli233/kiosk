...
@endphp
{!! Form::open(array('method'=>$method,'id'=>'bnk_pymnt_form','enctype'=>'multipart/form-data')) !!}
    ...
    {{  Form::hidden('custom_id', (isset($data->custom_id) ? $data->custom_id : 0) , array("class" => "bpv_custom_id")) }}
                ...
                <div class="card-header">
                    <div class="row">
                        <div class="col-lg-2 col-md-2 col-sm-12 col-xs-12 col_head">
                            ...
                            {!! Form::select("bank_id", ["Please select"]+hp_banks(),isset($data->account_id) ? ($data->account_id): null, array("class" => "form-control select2 cls_bank_id", 'required')) !!}
                        </div>
            
                        <div class="col-lg-2 col-md-2 col-sm-12 col-xs-12 col_head">
                            {!! Html::decode(Form::label('method','Cheque#')) !!} </br>
                            {!! Form::text('cheque_no',(isset($data->cheque_no) ? ($data->cheque_no): null), array('placeholder' => 'Cheque#', 'id' => 'cheque_no','class' => 'form-control', 'required' )) !!}
                        </div> 

                        <div class="col-lg-2 col-md-2 col-sm-12 col-xs-12 col_head">
                            {!! Html::decode(Form::label('cheque_amount','Cheque Amount')) !!} </br>
                            {!! Form::number('cheque_amount',(isset($data->ledger->amount) ? $data->ledger->amount: 0), array('placeholder' => 'Cheque amount', 'id' => 'cheque_amount','class' => 'form-control', 'required' )) !!}
                        </div> 
                                    ...
                                    <tbody>
                                        @if(isset($transactions))
                                            @foreach($transactions as $key => $transaction)
                                                <tr>
                                                    <td>
                                                        {!! Form::select("account_ids[]", ["Please select"]+hp_accounts() ,$transaction->account_id, array("class" => "form-control select2 cls_bnk_pymnt_account_ids")) !!}
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td>
                                                        {{ Form::number("amounts[]", $transaction->ledger->amount, array("placeholder" => "amounts","class" => "form-control cls_bnk_pymnt_amnt","min"=>0, "step"=>"any")) }}
                                                    </td>
                                                    <td>
...
<script>
    $(document).ready(function () {  
        ...
        var trnx_type_id    = {{ ((isset($data->transaction_type_id)) && ($data->transaction_type_id == 5)) ? $data->transaction_type_id : 0 }};
        ...
        function compareDebitCredit(){
            ...
            if (isNaN(amount)) { // Check if the value is not set or NaN
                amount = 0.00; // Set cih_balance to 0 if it's not set
            }else{
            ...
            var dbt_amnt        = 0;
            var dbt_amount      = 0;
            var dbt_inputs      = $(".cls_bnk_pymnt_amnt");
            ...
            // console.log("dbt_amount: ", dbt_amount);
